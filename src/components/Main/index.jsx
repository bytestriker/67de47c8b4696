...
const Main = () => {
  ...
  return (
    <>
      {getLoading && <Loading />}
      {warningData.modalAlert && <ModalInfoProject setAlert={setAlert} warningData={warningData} />}
      {storeTankModal.modalTank && <ModalBlockPlanet />}
      ...
      {popupstate.modalP ? (
        ...
        <main className={`${currentPathName}`}>
          ...
          <Route
            exact
            path="/login"
            render={() => (contextValue.isLogged() ? <Redirect to="/" /> : <Login />)}
          />
